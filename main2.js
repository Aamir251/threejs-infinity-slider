...
const renderTargetOne = new THREE.WebGLRenderTarget(sizes.width, sizes.height, {
  format : THREE.RGBAFormat,
  magFilter : THREE.NearestFilter,
  minFilter : THREE.NearestFilter
})
...
const updateMeshes = () =>
{
  ...
  meshes.forEach(item => {
    ...
    // item.mesh.position.x =  ((item.index * sizeofMesh *  spacingBetweenSlides + positionFactor + currentScroll * sizeofMesh) % totalWidth + totalWidth) % totalWidth + positionFactor - sizeofMesh

    // item.mesh.position.x =  ((item.index * sizeofMesh *  spacingBetweenSlides + currentScroll * sizeofMesh + sizeofMesh) % totalWidth + totalWidth) % totalWidth + positionFactor - sizeofMesh

  })
  // console.log("totalWidth ", totalWidth);
  // console.log("Position: ", meshes[0].mesh.position.x);
}
